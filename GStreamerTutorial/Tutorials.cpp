...
int Tutorials::tutorial_2()
{
	...
	if (gst_element_link(vertigo, sink) != TRUE) {
		g_printerr("Elements could not be linked.\n");
		gst_object_unref(pipeline);
		return -1;
	}
...
int Tutorials::tutorial_3()
{
	...
	if (!data.pipeline || !data.source || !data.videoConvert || !data.videoSink || !data.audioConvert || !data.audioSink) {
		g_printerr("Not all elements could be created.\n");
		return -1;
	}
	...
	gst_bin_add_many(GST_BIN(data.pipeline), data.source, data.videoConvert, data.videoSink, data.audioConvert, data.audioSink, NULL);
...
void Tutorials::pad_added_handler(GstElement *src, GstPad *new_pad, CustomData *data) {
